...
import { getBusinessProfileByOwner } from '../lib/businessProfileService.ts'
...
function DashboardPage() {
  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      const result = await getBusinessProfileByOwner(user.id)
      setProfile(result)
      setLoadState(result ? 'found' : 'empty')
    } catch (error) {
      console.error('Failed to load business profile for dashboard:', error)
      setLoadState('error')
    }
  }, [user])
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-slate-50">
      ...
      <main id="my-business" className="max-w-3xl mx-auto px-4 py-10">
        <div className="mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 tracking-tight mb-1.5">My Business</h1>
          <p className="text-sm text-gray-500">Manage your business profile from one place.</p>
        </div>
        ...
        {loadState === 'loading' && (
          <div className="flex items-center justify-center min-h-[40vh]" role="status" aria-live="polite">
            ...
            <span className="sr-only">Loading your business profile…</span>
          </div>
        )}
        ...
        {loadState === 'error' && (
          <div className="flex flex-col items-center justify-center min-h-[40vh] text-center px-4">
            ...
            <p className="text-gray-700 font-medium mb-6">Unable to load your business profile.</p>
            <button
              type="button"
              onClick={loadProfile}
        ...
        {loadState === 'found' && profile && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5 mb-8">
              <div>
                <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-1">Business Name</p>
